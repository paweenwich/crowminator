...
def draw_ground():
    ...
    # Draw grid
    glColor3f(0.2, 0.2, 0.2)  # Grid lines in dark gray
    glBegin(GL_LINES)
    for i in range(-5, 6):
        # Draw vertical lines
        glVertex3f(i, -1, -5)
...
def draw_cube(x, y, z, size):
    ...
    glColor3f(0.6, 0.3, 0.1)  # Brownish cube color
    glBegin(GL_QUADS)
...
def draw_shadow(x, z, size):
    # Shadow cast directly beneath the cube
    glPushMatrix()
    glTranslatef(x, -0.5, z)  # Position the shadow just below the cube
    glScalef(size, 0.01, size)  # Flatten the shadow along the Y-axis
    glColor3f(0.2, 0.2, 0.2)  # Dark gray shadow
    glBegin(GL_QUADS)
    
    # Draw shadow as a flat square beneath the cube
